...
With an analogous construction we get direct sum of maps $f \oplus g$.
...
\begin{definition}[Extension]
	...
	\[\begin{tikzcd}[sep=small]
		{\xi \colon} & 0 & A & {X_1} & {X_2} & \cdots & {X_n} & B & 0
		\arrow[from=1-2, to=1-3]
		\arrow[from=1-3, to=1-4]
		\arrow[from=1-4, to=1-5]
		\arrow[from=1-5, to=1-6]
...
\[\begin{tikzcd}[sep=small]
	{\xi \colon} & 0 & A & {X_1} & {X_2} & \cdots & {X_n} & B & 0 \\
	& 0 & A & {X''_1} & {X''_2} & \cdots & {X''_n} & B & {0} \\
	{\xi'\colon} & 0 & A & {X_1'} & {X_2'} & \cdots & {X'_n} & B & {0.}
	\arrow[from=1-2, to=1-3]
	\arrow[from=1-3, to=1-4]
	\arrow[from=1-4, to=1-5]
	\arrow[from=1-5, to=1-6]
	...
	\arrow["{\id_B}", from=2-8, to=1-8]
	\arrow[from=2-8, to=2-9]
	\arrow["{\id_B}"', from=2-8, to=3-8]
...
\begin{remark}
	If $n=1$, the relation is much simpler: in the following diagram both middle maps must necessarily be isomorphism, therefore we need only to look for a map $X \to X'$ that makes the diagram commute.
	\[\begin{tikzcd}[sep=small]
		{\xi \colon} & 0 & A & X & B & 0 \\
		& 0 & A & {X''} & B & 0 \\
		{\xi'\colon} & 0 & A & {X'} & B & 0
		...
		\arrow[from=2-4, to=2-5]
		\arrow[from=2-4, to=3-4]
		\arrow["{\id_B}", from=2-5, to=1-5]
		\arrow[from=2-5, to=2-6]
		\arrow["{\id_B}"', from=2-5, to=3-5]
		\arrow[from=3-2, to=3-3]
		\arrow[from=3-3, to=3-4]
		\arrow[from=3-4, to=3-5]
		\arrow[from=3-5, to=3-6]
...
We will now introduce an operation, called the \textit{Baer sum}, on $\Ext^n_\C(A,B)$, for $n\geq 1$. Let us begin bt defining this operation on $\Ext^1_\C(A,B)$. Let us begin by consider two equivalence classes of extensions of $A$ and $B$ and taking two representatives
...
We then set $X''$ as the following pullback:
